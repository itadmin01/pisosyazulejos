...
class StockMoveExtends(models.Model):
    _inherit = 'stock.move'
        ...
        for move in self:
            ...
            if picking:
                if picking.partner_id.id != move.partner_id.id or picking.origin != move.origin:
                    origin_list=[picking.origin,move.origin]
                    if not picking.origin:
                       origin_data = move.origin
                    elif not move.origin:
                       origin_data = picking.origin
                    else:
                       origin_data = ",".join(origin_list)
                    picking.write({
                        'partner_id': False,
                        'origin': origin_data,
                    })
